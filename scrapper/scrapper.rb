	...
	while i < j - 1
		cls_detail = arr[i].split " "
		 ...
		 cls_detail.each do |variable|
		 	if (variable.include? 'M' or variable.include? 'T' or variable.include? 'W' or variable.include? 'R' or variable.include? 'F') and variable.length < 2 and !day.include? variable	
		 		day << variable
